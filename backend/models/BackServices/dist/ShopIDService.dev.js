...
var url = "mongodb://localhost:27017/"; //TO INSERT DATA IN MONGODB
...
myrouter.get('/NewIDGenerate', function (req, res) {
  res.status(200).render('NewID.pug');
});
myrouter.get('/admin/Dashboard', function (req, res) {
...
myrouter.get('/admin/Dashboard/UpdateSlides', function (req, res) {
  if (req.isAuthenticated()) {
    usertype = req.user.Usertype;
    console.log(usertype);
...
myrouter.get('/admin/Dashboard/AddNewMember', function (req, res) {
  if (req.isAuthenticated()) {
    usertype = req.user.Usertype;
    console.log(usertype);
...
myrouter.get('/admin/Request', function (req, response) {
  if (req.isAuthenticated()) {
...
myrouter.get('/admin/Dashboard/AddPromocode', function (req, res) {
  if (req.isAuthenticated()) {
    usertype = req.user.Usertype;
    console.log(usertype);
    ...
    if (usertype == "admin") {
      ...
      MongoClient.connect(url, {
        ...
        if (err) throw err;
        var dbo = db.db("trial");
        dbo.collection("OrderID").find().sort({
          Status: -1
        }).limit(50).skip(skip2).toArray(function (err, result) {
          if (result != "") {
            if (err) throw err;
            var params = {
              'newOBJ': result,
      ...
      MongoClient.connect(url, {
        ...
        dbo.collection("OrderID").find().sort({
          ...
          if (result != "") {
            ...
            res.status(200).render('ad_dashboard/ad_underReview.pug', {
              'result': 'No Orders Right Now'
            });
          }
        });
  ...
  if (req.isAuthenticated()) {
    ...
    if (usertype == "admin") {
      ...
      MongoClient.connect(url, {
        ...
        if (err) throw err;
        var dbo = db.db("trial");
        dbo.collection("EMPLOYEE_ID").find({
          ...
          if (result != "") {
            ...
            res.status(200).render('admin_dashboard/ad_underReview.pug', {
              'result': 'No Request Right Now'
            });
          }
        });
      });
...
myrouter.get('/admin/Dashboard/Shoprecords', function (req, res) {
  if (req.isAuthenticated()) {
    skip2 = 0;
    next = parseInt(req.query.page);
    ...
    if (usertype == "admin") {
      MongoClient.connect(url, {
        ...
        if (err) throw err;
        var dbo = db.db("trial");
        dbo.collection("EMPLOYEE_ID").find().sort({
          emp_request: 1
        }).limit(50).skip(skip2).toArray(function (err, result) {
          if (result != "") {
            if (err) throw err;
